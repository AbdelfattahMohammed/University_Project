...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('timetables', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('department_id');// لازم يكون من نوع string زي الـ departments table
            $table->foreign('department_id')->references('id')->on('departments')->onDelete('cascade');
            $table->integer('year');
            $table->foreignId('room_id')->nullable()->constrained()->onDelete('set null'); // السنة (1, 2, 3, 4)
